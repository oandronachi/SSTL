...
SSTL_MAP_DECLARE(probe_map, int, int, 2, sstl_cmp_int_v)
SSTL_MAP_DEFINE(probe_map, int, int, 2, sstl_cmp_int_v)

SSTL_SET_DECLARE(probe_set, int, 2, sstl_cmp_int_v)
SSTL_SET_DEFINE(probe_set, int, 2, sstl_cmp_int_v)
...
SSTL_UNORDERED_MAP_DECLARE(probe_umap, int, int, 2, 5, sstl_eq_int_v, sstl_hash_int_v)
SSTL_UNORDERED_MAP_DEFINE(probe_umap, int, int, 2, 5, sstl_eq_int_v, sstl_hash_int_v)

SSTL_UNORDERED_SET_DECLARE(probe_uset, int, 2, 5, sstl_eq_int_v, sstl_hash_int_v)
SSTL_UNORDERED_SET_DEFINE(probe_uset, int, 2, 5, sstl_eq_int_v, sstl_hash_int_v)
...
int main(void) {
  ...
  if (!probe_str_pop_back(&str) || probe_str_empty(&str) || !probe_str_pop_back(&str) || !probe_str_empty(&str)) return 46;
  ...
  if (probe_deque_try_push_back(&deque, 9) || !probe_deque_pop_back(&deque, &out) || out != 8) return 66;
...

  {
    ...
    if (!probe_queue_push(&queue, 1) || !probe_queue_push(&queue, 2) || !probe_queue_pop(&queue, &out) || out != 1) return 20;
    if (!probe_stack_push(&stack, 1) || !probe_stack_push(&stack, 2) || !probe_stack_pop(&stack, &out) || out != 2) return 21;
    if (!probe_pqueue_push(&pqueue, 1) || !probe_pqueue_push(&pqueue, 3) || !probe_pqueue_push(&pqueue, 2)) return 22;
    if (*probe_pqueue_top(&pqueue) != 3 || !probe_pqueue_pop(&pqueue, &out) || out != 3) return 23;
  }
  ...
  probe_map map;
  probe_map_init(&map);
  if (probe_map_capacity(&map) != 2u || !probe_map_insert(&map, 1, 10) || *probe_map_find(&map, 1) != 10) return 8;

  probe_set set;
  probe_set_init(&set);
  if (probe_set_capacity(&set) != 2u || !probe_set_insert(&set, 3) || *probe_set_find(&set, 3) != 3) return 9;
  ...
  hash = sstl_hash_bytes_v(sample, sizeof(sample));
  {
    ...
    if (sstl_cmp_int_v(&one, &two) >= 0 || sstl_cmp_uint_v(&two_u, &one_u) <= 0 || sstl_cmp_str_v(&a_text, &b_text) >= 0 ||
        !sstl_eq_int_v(&one, &one) || sstl_eq_uint_v(&one_u, &two_u) || !sstl_eq_str_v(&same_text, &same_text) || hash == 0u) return 19;
  }
