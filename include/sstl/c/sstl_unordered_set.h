...
#define SSTL_UNORDERED_SET_DECLARE(NAME, K, CAP, BUCKETS, EQ, HASH) \
   ...
   */ \
  SSTL_C_INLINE size_t NAME##_bucket_count(const NAME* s); \
  SSTL_C_INLINE float NAME##_load_factor(const NAME* s); \
  SSTL_C_INLINE float NAME##_max_load_factor(const NAME* s); \
  /** \
   ...
   */ \
  SSTL_C_INLINE size_t NAME##_find_slot(const NAME* s, K key); \
  /** \
  ...
  SSTL_C_INLINE NAME##_iterator NAME##_begin(const NAME* s); \
  SSTL_C_INLINE NAME##_iterator NAME##_end(const NAME* s); \
  SSTL_C_INLINE NAME##_iterator NAME##_next(const NAME* s, NAME##_iterator it); \
  SSTL_C_INLINE K* NAME##_deref(NAME* s, NAME##_iterator it); \
  SSTL_C_INLINE NAME##_iterator NAME##_equal_range_first(const NAME* s, K key); \
  SSTL_C_INLINE NAME##_iterator NAME##_equal_range_second(const NAME* s, K key); \
  /** \
...
#define SSTL_UNORDERED_SET_DEFINE(NAME, K, CAP, BUCKETS, EQ, HASH) \
   ...
   * @return Result described by the function brief. \
   */ \
  SSTL_C_INLINE void NAME##_init(NAME* s) { size_t i; s->free_head = (CAP) == 0 ? (size_t)-1 : 0u; for (i = 0u; i != (size_t)((BUCKETS) == 0 ? 1 : (BUCKETS)); ++i) s->buckets[i] = (size_t)-1; for (i = 0u; i != (size_t)((CAP) == 0 ? 1 : (CAP)); ++i) { s->next[i] = (i + 1u < (size_t)(CAP)) ? i + 1u : (size_t)-1; s->used[i] = false; } s->size = 0u; } \
  /** \
   * @brief Return the number of live keys. \
   * @param s String or set instance. \
   * @return The number of live keys. \
  ...
  SSTL_C_INLINE void NAME##_clear(NAME* s) { NAME##_init(s); } \
   ...
   * @return The compile-time bucket count. \
   */ \
  SSTL_C_INLINE size_t NAME##_bucket_count(const NAME* s) { (void)s; return (size_t)(BUCKETS); } \
  SSTL_C_INLINE float NAME##_load_factor(const NAME* s) { return (BUCKETS) == 0 ? 0.0f : (float)s->size / (float)(BUCKETS); } \
  SSTL_C_INLINE float NAME##_max_load_factor(const NAME* s) { (void)s; return (BUCKETS) == 0 ? 0.0f : (float)(CAP) / (float)(BUCKETS); } \
  /** \
   * @brief Return the bucket index for `key`, or zero when the bucket count is zero. \
   * @param key Lookup or insertion key. \
   * @return The bucket index for `key`, or zero when the bucket count is zero. \
   ...
   * @return The live slot containing `key`, or the null slot sentinel when absent. \
   */ \
  SSTL_C_INLINE size_t NAME##_find_slot(const NAME* s, K key) { size_t cur; if ((BUCKETS) == 0) return (size_t)-1; cur = s->buckets[NAME##_bucket_index(key)]; while (cur != (size_t)-1) { if (s->used[cur] && (EQ)((const void*)&s->data[cur], (const void*)&key)) return cur; cur = s->next[cur]; } return (size_t)-1; } \
  /** \
   * @brief Return the stored key matching `key`, or null when absent. \
   * @param s String or set instance. \
   * @param key Lookup or insertion key. \
   * @return The stored key matching `key`, or null when absent. \
   */ \
  SSTL_C_INLINE K* NAME##_find(NAME* s, K key) { size_t slot = NAME##_find_slot(s, key); return slot == (size_t)-1 ? 0 : &s->data[slot]; } \
  SSTL_C_INLINE size_t NAME##_count(NAME* s, K key) { return NAME##_find_slot(s, key) == (size_t)-1 ? 0u : 1u; } \
  SSTL_C_INLINE NAME##_iterator NAME##_begin(const NAME* s) { size_t i; for (i = 0u; i != (size_t)(CAP); ++i) if (s->used[i]) return i; return (size_t)-1; } \
  SSTL_C_INLINE NAME##_iterator NAME##_end(const NAME* s) { (void)s; return (size_t)-1; } \
  SSTL_C_INLINE NAME##_iterator NAME##_next(const NAME* s, NAME##_iterator it) { size_t i; if (it == (size_t)-1) return (size_t)-1; for (i = it + 1u; i != (size_t)(CAP); ++i) if (s->used[i]) return i; return (size_t)-1; } \
  SSTL_C_INLINE K* NAME##_deref(NAME* s, NAME##_iterator it) { return it < (size_t)(CAP) && s->used[it] ? &s->data[it] : 0; } \
  SSTL_C_INLINE NAME##_iterator NAME##_equal_range_first(const NAME* s, K key) { return NAME##_find_slot(s, key); } \
  SSTL_C_INLINE NAME##_iterator NAME##_equal_range_second(const NAME* s, K key) { NAME##_iterator first = NAME##_equal_range_first(s, key); return first == (size_t)-1 ? (size_t)-1 : NAME##_next(s, first); } \
  /** \
   * @brief Insert `key` into the hashed bucket chain. \
   * @param s String or set instance. \
   * @param key Lookup or insertion key. \
