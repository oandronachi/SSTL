...
#define SSTL_UNORDERED_MAP_DECLARE(NAME, K, V, CAP, BUCKETS, EQ, HASH) \
   ...
   */ \
  static SSTL_C_UNUSED size_t NAME##_find_slot(NAME* m, K key); \
  /** \
   ...
   */ \
  static SSTL_C_UNUSED V* NAME##_find(NAME* m, K key); \
  /** \
   ...
   */ \
  static SSTL_C_UNUSED bool NAME##_insert(NAME* m, K key, V value); \
  /** \
   ...
   * @return Result described by the function brief. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_erase(NAME* m, K key);
...
#define SSTL_UNORDERED_MAP_DEFINE(NAME, K, V, CAP, BUCKETS, EQ, HASH) \
   ...
   * @return Result described by the function brief. \
   */ \
  static SSTL_C_UNUSED void NAME##_init(NAME* m) { size_t i; m->free_head = (CAP) == 0 ? (size_t)-1 : 0u; for (i = 0u; i != (size_t)((BUCKETS) == 0 ? 1 : (BUCKETS)); ++i) m->buckets[i] = (size_t)-1; for (i = 0u; i != (size_t)((CAP) == 0 ? 1 : (CAP)); ++i) { m->next[i] = (i + 1u < (size_t)(CAP)) ? i + 1u : (size_t)-1; m->used[i] = false; } m->size = 0u; } \
  /** \
   * @brief Return the number of live entries. \
   * @param m Map instance. \
   * @return The number of live entries. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_size(const NAME* m) { return m->size; } \
  /** \
   * @brief Return the fixed entry capacity. \
   * @param m Map instance. \
   * @return The fixed entry capacity. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_capacity(const NAME* m) { (void)m; return (size_t)(CAP); } \
  /** \
   * @brief Return true when no live entries are present. \
   * @param m Map instance. \
   * @return True when no live entries are present. \
   */ \
  static SSTL_C_UNUSED bool NAME##_empty(const NAME* m) { return m->size == 0u; } \
  /** \
   * @brief Return true when no free entry slots remain. \
   * @param m Map instance. \
   * @return True when no free entry slots remain. \
   */ \
  static SSTL_C_UNUSED bool NAME##_full(const NAME* m) { return m->size == (size_t)(CAP); } \
  /** \
   * @brief Return the compile-time bucket count. \
   * @param m Map instance. \
   * @return The compile-time bucket count. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_bucket_count(const NAME* m) { (void)m; return (size_t)(BUCKETS); } \
  /** \
   * @brief Return the bucket index for `key`, or zero when the bucket count is zero. \
   * @param key Lookup or insertion key. \
   * @return The bucket index for `key`, or zero when the bucket count is zero. \
   ...
   * @return The live slot containing `key`, or the null slot sentinel when absent. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_find_slot(NAME* m, K key) { size_t cur; if ((BUCKETS) == 0) return (size_t)-1; cur = m->buckets[NAME##_bucket_index(key)]; while (cur != (size_t)-1) { if (m->used[cur] && (EQ)((const void*)&m->data[cur].key, (const void*)&key)) return cur; cur = m->next[cur]; } return (size_t)-1; } \
  /** \
   * @brief Return the value for `key`, or null when absent. \
   * @param m Map instance. \
   * @param key Lookup or insertion key. \
   * @return The value for `key`, or null when absent. \
   */ \
  static SSTL_C_UNUSED V* NAME##_find(NAME* m, K key) { size_t slot = NAME##_find_slot(m, key); return slot == (size_t)-1 ? 0 : &m->data[slot].value; } \
  /** \
   * @brief Insert `key`/`value` into the hashed bucket chain. \
   * @param m Map instance. \
   * @param key Lookup or insertion key. \
