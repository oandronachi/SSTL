...
#define SSTL_FLAT_MAP_DECLARE(NAME, K, V, CAP, CMP) \
   ...
   */ \
  static SSTL_C_UNUSED size_t NAME##_lower_bound(const NAME* m, K key); \
  /** \
   ...
   */ \
  static SSTL_C_UNUSED NAME##_pair* NAME##_at_index(NAME* m, size_t index); \
  /** \
...
#define SSTL_FLAT_MAP_DEFINE(NAME, K, V, CAP, CMP) \
   ...
   * @return Result described by the function brief. \
   */ \
  static SSTL_C_UNUSED void NAME##_init(NAME* m) { m->size = 0u; } \
  /** \
   * @brief Return the number of live key/value entries. \
   * @param m Map instance. \
   * @return The number of live key/value entries. \
   ...
   * @return The first index whose key is not less than `key`. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_lower_bound(const NAME* m, K key) { size_t i; for (i = 0u; i != m->size; ++i) if ((CMP)((const void*)&m->data[i].key, (const void*)&key) >= 0) return i; return m->size; } \
  /** \
   * @brief Return the value for `key`, or null when absent. \
   * @param m Map instance. \
   * @param key Lookup or insertion key. \
   * @return The value for `key`, or null when absent. \
   */ \
  static SSTL_C_UNUSED V* NAME##_find(NAME* m, K key) { size_t i = NAME##_lower_bound(m, key); return i != m->size && (CMP)((const void*)&key, (const void*)&m->data[i].key) == 0 ? &m->data[i].value : 0; } \
  /** \
   * @brief Return the pair at `index`, or null when `index` is outside the live range. \
   * @param m Map instance. \
   * @param index Caller-supplied argument used by this operation. \
   * @return The pair at `index`, or null when `index` is outside the live range. \
   */ \
  static SSTL_C_UNUSED NAME##_pair* NAME##_at_index(NAME* m, size_t index) { return index < m->size ? &m->data[index] : 0; } \
  /** \
   * @brief Insert `key`/`value` while preserving sorted order. \
   * @param m Map instance. \
   * @param key Lookup or insertion key. \
   ...
   * @return `true` when the documented condition holds; otherwise `false`. \
   */ \
  static SSTL_C_UNUSED bool NAME##_insert(NAME* m, K key, V value) { size_t i; size_t pos = NAME##_lower_bound(m, key); if (pos != m->size && (CMP)((const void*)&key, (const void*)&m->data[pos].key) == 0) return false; if (m->size == (size_t)(CAP)) { SSTL_C_PANIC("flat_map full"); return false; } for (i = m->size; i != pos; --i) m->data[i] = m->data[i - 1u]; m->data[pos].key = key; m->data[pos].value = value; ++m->size; return true; } \
  /** \
   * @brief Erase `key` and return the number of entries removed. \
   * @param m Map instance. \
   * @param key Lookup or insertion key. \
   * @return Result described by the function brief. \
   */ \
  static SSTL_C_UNUSED size_t NAME##_erase(NAME* m, K key) { size_t i; size_t pos = NAME##_lower_bound(m, key); if (pos == m->size || (CMP)((const void*)&key, (const void*)&m->data[pos].key) != 0) return 0u; for (i = pos + 1u; i != m->size; ++i) m->data[i - 1u] = m->data[i]; --m->size; return 1u; }
